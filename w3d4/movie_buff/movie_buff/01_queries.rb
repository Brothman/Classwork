...
def harrison_ford
  ...
  #
  # movie_id = Actor.joins(:castings)
  #             .where(<<-SQL, 'Harrison Ford', 1)
  #               actors.name = ? AND castings.ord != ?
  #             SQL
  #             .pluck("castings.movie_id")
  #
  # Actor.joins(:movies)
  #   .where(<<-SQL, "Harrison Ford", movie_id)
  #     actors.name NOT LIKE ? AND movies.id IN (?)
  #   SQL
  #   .select("movies.title, movies.id").distinct

  # movies = Movie.includes(:actors)
  #           .where(<<-SQL, 'Harrison Ford')
  #
  #           SQL

  Movie.joins(:actors)
    .where("actors.name LIKE 'Harrison Ford'")
...
def directed_by_one_of(them)
  ...
  Movie.joins(:director).where('actors.name IN (?)', them).select(:id, :title)
end
